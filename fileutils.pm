...
#
# $Id: fileutils.pm,v 1.6 2002/05/01 17:17:37 chris Exp $
...
=head1 SYNOPSIS
 ...
 use fileutils (getlock releaselock fetchfile createfile 
...
sub getlock($;$) {
	...
	my $lock = \*{"fileutils::_SYMBOL" . $genseq};
...
sub releaselock($) {
	...
	no strict 'refs';
	my $lock = \*{"fileutils::_SYMBOL" . $no};
	close $lock or croak "Could not release lock, already released?";
	delete $fileutils::{"_SYMBOL".$no} or die "??";
}
...
calling fetchfile. 
Doesn't read files bigger than 1MB (can be changed through $fileutils::maxfilesize).
...
directory with $fileutils::tmpsuffix (defaults to ".new~%"
...
with appendix $fileutils::bcksuffix (which defaults to ".bck"). Without 'backupfilepath'
and with  $fileutils::bcksuffix set to undef, no backup will be made.
Croaks on errors or if the file doesn't exist.
...
or by setting $fileutils::checkperms to false.
...
strictperms or $fileutils::strictperms.
...
If $fileutils::bcksuffix is set, tries to rename the given file to it's backup,
...
sub getfilehandle($) {
	my ($file)=@_;
	no strict 'refs';
	my $in = \*{"fileutils::_SYMBOL" . $genseq};
	sysopen $in,$file, O_RDONLY or croak "Could not open $file for reading: $!";
	delete $fileutils::{"_SYMBOL".$genseq++} or die "??";
	$in
}
...
sub createfile($;$$) {
	...
	eval {
		if (defined $_[2]) {
			if (ref $_[2]) {
				defined syswrite OUT, ${$_[2]} or croak "Error writing to $file: $!";
			} else {
				defined syswrite OUT, $_[2] or croak "Error writing to $file: $!";
			}
		}
...
sub writebackfile($$;@) {
	...
	# create new file
	my $tmpfile;
	croak "writebackfile: \$fileutils::tmpsuffix is undefined or empty" unless defined $tmpsuffix and length($tmpsuffix);
	if ($tmpsuffix=~ /\%/) {
		my $tries=0;
		TRY: {
			$tmpfile= $tmpsuffix;
	...
	eval {
		...
		if (ref $$buffer) {
			defined syswrite OUT, $$$buffer or croak "Error writing to $tmpfile: $!";
		} else {
			defined syswrite OUT, $$buffer or croak "Error writing to $tmpfile: $!";
		}
		chmod $oldmode & (
			(defined $lrecreatesetid ? $lrecreatesetid : $recreatesetid) ? 07777 : 01777 # allow sticky bit?
		), $tmpfile or die "??? $tmpfile: $!"; # do it *after* the syswrite or linux will delete the suid bit again
