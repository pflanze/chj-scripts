...
sub xtmpfile {
    my $proto=shift;
    my ($maybe_basepath,$mode,$autoclean,$mkdircb)=@_;  # basepath: /dir/startofname  (or: /dir/   but not /dir ), or a function which receives an autogenerated basepath and a random number and should return the path to use
    my $genpath= do{
	my $gen_base= sub {
	    my $n=$0;
	    $n=~tr/\//-/;
...
#    local $@; nein hier nicht. weil sonst geht das eigene die dann nicht...
	...
	if ($@) {
		...
		if (++$tries < $MAXTRIES) {
		    ...
		    croak "xtmpfile: too many attempts to create a tempfile, last attempt was '$last_path'";
		}
	    } elsif ($Chj::IO::ERRNO== ENOENT and $mkdircb) {
		#warn "JEEEEE";
		#use Chj::repl;repl;
		if ($called_mkdircb) {
		    croak "xtmpfile: got ENOENT but mkdir-callback has already been called, for attempt '$last_path'";
		} else {
		    #&$mkdircb;
		    $mkdircb->(&$genpath(""));
		    $called_mkdircb=1;
