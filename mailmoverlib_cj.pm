...
sub analyze_file($ ; $ ) {
	...
	my $msgid= &$messageid;
	if (!defined $msgid) {
	    if (defined(my $spamhits= $head->spamhits)) {
		if ($spamhits > 0) {
		  T:{
...
# 		    $foldername= "AUTOREPLY";
		...
		} elsif ($from eq 'GMX Magazin <mailings@gmx.net>') {
		    $foldername= "GMX Magazin"; $type="list";
		} elsif ($from=~ /GMX Spamschutz.* <mailings\@gmx.net>/) {
		...
		# VIREN!:
		elsif ( do{ $DB::single=1;
$subject eq 'New Internet Critical Pack'
		       or $subject=~ /^\!VIRUS:/
...
sub save_is_own {
    my ($mail) = @_;
    my $ownmsgidtable= Chj::FileStore::PIndex->new($ownmsgid_base);
    $ownmsgidtable->add(scalar pick_out_of_anglebrackets($mail->header("message-id")),"");
    if (my $subj= _eingestampftessubject($mail)) {
	my $ownsubjectstable= Chj::FileStore::PIndex->new($ownsubjects_base);
